...
void main() {
  group(r'\epsilon\frac 1 3', () {
    ...
    test('correctly passes token to symbol', () {
      expect(
        ((rootNode as GroupNode).children[0]! as SymbolNode).context.input,
        equals(r'\epsilon'),
      );
    });
    ...
    test('correctly passes two nodes to frac', () {
      expect(
        ((rootNode as GroupNode).children[1]! as MultiChildNode).children,
        hasLength(2),
      );
    });
  ...
  group(r'\epsilon = \frac 2 {3 + 2}', () {
    final rootNode =
        Parser(r'\epsilon = \frac 2 {3 + 2}').parse() as GroupNode?;
  ...
  group(r'\varepsilon = \frac{\frac{2}{1}}{3}', () {
    ...
    late List<ParsingNode?> children;
