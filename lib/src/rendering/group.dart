...
class RenderGroup extends RenderNode {
  ...
  void configure() {
    ...
    var heightTopOverflow = .0;
    var heightBottomOverflow = .0;
    ...
    for (final child in children) {
      ...
      final dyShifted = dy + _subSupAddend(child) + _raiseBoxAddend(child);
      ...
      child.positionNode(
        Offset(
          width + symbolSpacing + dxShift,
          dyShifted,
        ),
      );
      width += size.width + symbolSpacing + dxShift;
      ...
      if (child is RenderSubSup) {
        if (CharacterCategory.superscript.matches(child.context.input!)) {
          ...
          heightBottomOverflow = max(
            heightBottomOverflow,
            (dyShifted + size.height) - initialHeight,
          );
        }
  ...
  static const _supFactor = 0.28;
  static const _subFactor = 0.238;
